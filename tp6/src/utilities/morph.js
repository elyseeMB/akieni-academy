...
export function morph(oldTree, newTree, options = MORPH_OPTIONS) {
  ...
  if (options.childrenOnly)
    return updateChildren(newTree, oldTree, options), oldTree;
...
function getNodeKey(node, options) {
  return (
    options?.getNodeKey?.(node) ?? (node instanceof Element ? node.id : void 0)
  );
}
