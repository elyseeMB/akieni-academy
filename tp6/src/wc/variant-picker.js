import { VariantSelectedEvent, VariantUpdateEvent } from "../theme/event.js";
import { MORPH_OPTIONS, morph } from "../utilities/morph.js";
import {
...
export default class VariantPicker extends Component {
  ...
  connectedCallback() {
    super.connectedCallback(),
      (this.refs.fieldsets || []).forEach((fieldset) => {
    ...
    if (urlVariantId) {
      const preselectElement = this.querySelector(
        `[data-variant-id="${urlVariantId}"]`,
      );
      preselectElement && this.updateSelectedOption(preselectElement);
    } else {
      this.#checkedIndices = [];
      for (const radios of this.#radios)
        for (const radio of radios)
          (radio.checked = !1),
            radio.removeAttribute("checked"),
            radio.removeAttribute("data-current-checked"),
            radio.removeAttribute("data-previous-checked");
      for (const select of Array.from(this.querySelectorAll("select"))) {
  ...
  variantChanged(event) {
    ...
    const isOnProductPage =
      ...
      isOnFeaturedProductSection = !!this.closest(
        "featured-product-information",
      ),
      morphElementSelector = loadsNewProduct
        ? "main"
  ...
  #getFieldsetMeasurements(fieldsetIndex) {
    const fieldset = (this.refs.fieldsets || [])[fieldsetIndex],
      checkedIndices = this.#checkedIndices[fieldsetIndex],
      radios = this.#radios[fieldsetIndex];
  ...
  updateFieldsetCss(fieldsetIndex) {
    ...
    measurements && this.#applyFieldsetMeasurements(measurements);
  }
  updateSelectedOption(target) {
    ...
    if (target instanceof HTMLInputElement) {
      const fieldsetIndex = Number.parseInt(target.dataset.fieldsetIndex || ""),
        inputIndex = Number.parseInt(target.dataset.inputIndex || "");
      if (!Number.isNaN(fieldsetIndex) && !Number.isNaN(inputIndex)) {
        ...
        if (radios && checkedIndices && fieldset) {
          ...
          const newCurrentIndex = checkedIndices[0],
            newPreviousIndex = checkedIndices[1];
          newCurrentIndex !== void 0 &&
            radios[newCurrentIndex] &&
            (radios[newCurrentIndex].dataset.currentChecked = "true"),
            newPreviousIndex !== void 0 &&
    ...
    if (target instanceof HTMLSelectElement) {
      const newValue = target.value,
        newSelectedOption = Array.from(target.options).find(
  ...
  buildRequestUrl(
    ...
    const params = [],
      viewParamValue = getViewParameterValue();
    viewParamValue && params.push(`view=${viewParamValue}`),
    ...
    const SECTION_ID_MAP = {
      ...
      closestSectionId = Object.keys(SECTION_ID_MAP).find((sectionId) =>
        this.closest(sectionId),
      );
    return closestSectionId
      ? (productUrl?.includes("?") && (productUrl = productUrl.split("?")[0]),
        `${productUrl}?section_id=${SECTION_ID_MAP[closestSectionId]}&${params.join("&")}`)
    ...
    this.#abortController?.abort(),
      ...
      fetch(requestUrl, { signal: this.#abortController.signal })
        ...
        .then((responseText) => {
          ...
          if (textContent)
            if (morphElementSelector === "main") this.updateMain(html);
  ...
  updateVariantPicker(newHtml) {
    ...
    return (
      morph(this, newVariantPickerSource, {
        ...MORPH_OPTIONS,
        getNodeKey: (node) =>
          node instanceof HTMLElement ? node.dataset.key : void 0,
      }),
  ...
  updateVariantPickerCss() {
    const measurements = (this.refs.fieldsets || [])
  ...
  updateElement(newHtml, elementSelector) {
    const element = this.closest(elementSelector),
      newElement = newHtml.querySelector(elementSelector);
...
customElements.get("variant-picker") ||
