...
const ERROR_MESSAGE_DISPLAY_DURATION = 1e4,
  ERROR_BUTTON_REENABLE_DELAY = 1e3,
  SUCCESS_MESSAGE_DISPLAY_DURATION = 5e3;
...
export class AddToCartComponent extends Component {
  ...
  handleClick(event) {
    ...
    const quantitySelector = this.closest("product-form-component")?.refs
      .quantitySelector;
    (quantitySelector?.canAddToCart &&
      !quantitySelector.canAddToCart().canAdd) ||
      (this.refs.addToCartButton.dataset.puppet !== "true" &&
        (this.dataset.addToCartAnimation === "true" &&
          !event.target.closest(".quick-add-modal") &&
  ...
  animateAddToCart = async function () {
    const { addToCartButton } = this.refs;
    this.#resetTimeouts || (this.#resetTimeouts = []),
      this.#resetTimeouts.forEach((timeoutId2) => clearTimeout(timeoutId2)),
      (this.#resetTimeouts = []),
    ...
    const timeoutId = setTimeout(() => {
      ...
      index > -1 && this.#resetTimeouts.splice(index, 1);
    }, 800);
...
class ProductFormComponent extends Component {
  ...
  #updateCartQuantityFromData(cart) {
    ...
    const cartItem = cart.items.find(
      ...
      quantitySelector = this.querySelector("quantity-selector-component");
    return (
  ...
  async #fetchAndUpdateCartQuantity() {
    if (!this.querySelector('input[name="id"]')?.value) return 0;
    try {
      const cart = await (await fetch("/cart.js")).json();
      return this.#updateCartQuantityFromData(cart);
    } catch (error) {
      return console.error("Failed to fetch cart quantity:", error), 0;
    }
  }
  ...
  handleSubmit(event) {
    ...
    event.preventDefault(), this.#timeout && clearTimeout(this.#timeout);
    const allAddToCartContainers = this.querySelectorAll(
      "add-to-cart-component",
    );
    if (
      Array.from(allAddToCartContainers).some(
    ...
    if (this.refs.quantitySelector?.canAddToCart) {
      ...
      if (!validation.canAdd) {
        ...
        const errorMessage = (this.dataset.quantityErrorMax || "").replace(
          "{{ maximum }}",
  ...
  #onVariantUpdate = async (event) => {
    ...
    const {
      ...
      newQuantityInput = event.detail.data.html.querySelector(
    ...
    const newQuantityRules =
      ...
      isQuantityRulesChanging = !!quantityRules != !!newQuantityRules,
      newPricePerItem = event.detail.data.html.querySelector("price-per-item");
    if (
      (isQuantityRulesChanging || !!pricePerItem != !!newPricePerItem) &&
      quantitySelector
    ) {
      const currentQuantityValue = quantitySelector.getValue?.(),
        newProductFormButtons = event.detail.data.html.querySelector(
          ".product-form-buttons",
        );
      if (productFormButtons && newProductFormButtons) {
        morph(productFormButtons, newProductFormButtons);
        const newQuantityInputElement = event.detail.data.html.querySelector(
          'quantity-selector-component input[ref="quantityInput"]',
        );
    ...
    const currentVolumePricing = this.refs.volumePricing,
      newVolumePricing = event.detail.data.html.querySelector("volume-pricing");
    this.#morphOrUpdateElement(
